...
invading an attractor from half B (and vice versa -- exhaustive combinations).
...
Master workflow:
    ...
    3. What happens if two stable communities are combined and allowed to
        restabilize? (THIS FILE and invasions_random.py)
...
# if os.path.exists('/to/path/attractors_comb_whole_'+str(PBS_ITERATOR)+'.data'):
...
# Write to file ----------------------------------------------------------------
f = open(r'attractors_comb_whole_'+str(PBS_ITERATOR)+'.data','wb')
#f = open(r'/to/path/attractors_comb_whole_'+str(PBS_ITERATOR)+'.data','wb')
